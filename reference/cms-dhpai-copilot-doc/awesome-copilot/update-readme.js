...
// Compute the base RAW URL for this repository so install buttons point to the correct files.
// Priority: REPO_RAW_BASE_URL env override -> derive from package.json repository.url
function computeRepoRawBaseUrl() {
	// 1) Allow explicit override (useful for custom deployments)
	if (process.env.REPO_RAW_BASE_URL) {
		return process.env.REPO_RAW_BASE_URL.replace(/\/$/, "");
	}

	// 2) Derive from package.json repository.url
	...
	try {
		...
		// Normalize common git URL formats
		// Examples:
		//  - https://github.com/owner/repo.git
		//  - git@github.com:owner/repo.git
		//  - https://hagithub.home/CMS/cms-dhpai-copilot-doc.git (GitHub Enterprise)
		let host = "";
		let owner = "";
		let name = "";
		...
		// Raw base URL patterns:
		// - GitHub.com:            https://raw.githubusercontent.com/{owner}/{repo}/{branch}
		// - GitHub Enterprise:     https://{host}/raw/{owner}/{repo}/{branch}
		if (host && owner && name) {
			if (host === "github.com") {
				return `https://raw.githubusercontent.com/${owner}/${name}/${branch}/awesome-copilot`;
			}
			// Assume GitHub Enterprise raw pattern
			return `https://${host}/raw/${owner}/${name}/${branch}/awesome-copilot`;
		}
	} catch (err) {
		console.warn(
			"Could not derive repo raw base URL from package.json, falling back to public default.",
		);
	}

	// 3) Fallback: public awesome-copilot main branch (should rarely be used in this repo)
	return "https://raw.githubusercontent.com/github/awesome-copilot/main";
}
const repoBaseUrl = computeRepoRawBaseUrl();
