-- DROP PROCEDURE hkpmi.web_hkpmi_by_nokname_nomore(inout int4, in varchar, in varchar, in varchar, in int4, in varchar, in int4, inout refcursor);

CREATE OR REPLACE PROCEDURE hkpmi.web_hkpmi_by_nokname_nomore(INOUT pas_return_code integer, IN par_nok_name character varying, IN par_last_name character varying, IN par_last_hkid character varying, IN par_authority_code integer, IN par_hosp_code character varying, IN par_row_count integer DEFAULT 301, INOUT p_refcur refcursor DEFAULT NULL::refcursor)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    /* process NOK Name */
    SELECT CONCAT(RTRIM(par_nok_name), '%')
    INTO par_nok_name;
    ...
    OPEN p_refcur FOR SELECT p.hkid,
           p.patient_name,
           p.sex,
           p.dob,
           h.mrn,
    ...
    WHERE n.nok_name LIKE par_nok_name
      AND n.major_nok = 'Y'
...
ALTER PROCEDURE "web_hkpmi_by_nokname_nomore" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";