    ...
    ELSE
        BEGIN
            ...
            select rtrim(p.patient_key) patient_key, rtrim(p.hkid) hkid, rtrim(patient_name) patient_name,
            		...
            		da.area_name,
            		da.area_chi,
            		SUBSTRING(CONVERT(varchar, p.source_system_dtm, 140),1,23)
            		from patient p, #temp_patient_key_list l,
            		ccc_big5 c1, ccc_big5 c2, ccc_big5 c3,
            		ccc_big5 c4, ccc_big5 c5, ccc_big5 c6
            		, address_detail a, elderly_home_table e
            		, district d, district_area da
            		where p.patient_key = l.patient_key
            		...
            		and l.record_id      *= a.record_id
            		and l.record_id      *= e.eh_address_id
            		and p.district       *= d.district_code
            		and d.district_area *= da.area_code
            ...
            */
            ...
            select rtrim(p.patient_key) patient_key, rtrim(p.hkid) hkid, rtrim(patient_name) patient_name,
            		...
            		da.area_name,
            		da.area_chi,
            		SUBSTRING(to_char(p.source_system_dtm,'YYYY-MM-DD HH24:MI:SS'),1,23)
                from patient p
                JOIN t$temp_patient_key_list l ON p.patient_key = l.patient_key
                JOIN ccc_unicode c1 ON substring(p.cccode1,1,4) = c1.ccc_head and substring(p.cccode1,5,1) = c1.ccc_tail
                JOIN ccc_unicode c2 ON substring(p.cccode2,1,4) = c2.ccc_head and substring(p.cccode2,5,1) = c2.ccc_tail
                JOIN ccc_unicode c3 ON substring(p.cccode3,1,4) = c3.ccc_head and substring(p.cccode3,5,1) = c3.ccc_tail
                JOIN ccc_unicode c4 ON substring(p.cccode4,1,4) = c4.ccc_head and substring(p.cccode4,5,1) = c4.ccc_tail
                JOIN ccc_unicode c5 ON substring(p.cccode5,1,4) = c5.ccc_head and substring(p.cccode5,5,1) = c5.ccc_tail
                JOIN ccc_unicode c6 ON substring(p.cccode6,1,4) = c6.ccc_head and substring(p.cccode6,5,1) = c6.ccc_tail
                JOIN address_detail a ON l.record_id = a.record_id
                JOIN elderly_home_table e ON l.record_id = e.eh_address_id
                JOIN district d ON p.district = d.district_code
                JOIN district_area da ON d.district_area = da.area_code
                order by p.patient_key;
...
ALTER FUNCTION "pass_hkpmi_get_patients_detail" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";
