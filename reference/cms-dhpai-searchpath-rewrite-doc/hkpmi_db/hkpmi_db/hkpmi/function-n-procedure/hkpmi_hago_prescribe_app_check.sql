-- DROP PROCEDURE hkpmi.hkpmi_hago_prescribe_app_check(inout int4, in varchar, inout int4, inout varchar, inout refcursor);

CREATE OR REPLACE PROCEDURE hkpmi.hkpmi_hago_prescribe_app_check(INOUT pas_return_code integer, IN par_hkid character varying, INOUT par_code integer, INOUT par_status character varying, INOUT p_refcur refcursor)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    ...
    SELECT
        ...
        INTO var_patient_key, var_old_hkid, var_txn_type, var_move_status, var_dob, var_death_indicator, var_death_date;
    SELECT
...
ALTER PROCEDURE "hkpmi_hago_prescribe_app_check" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";
