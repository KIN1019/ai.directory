-- DROP PROCEDURE hkpmi.cpi_pq_validate_hkid(inout int4, in bpchar, inout bpchar);

CREATE OR REPLACE PROCEDURE hkpmi.cpi_pq_validate_hkid(INOUT pas_return_code integer, IN par_hkid VARCHAR, INOUT par_valid_flag VARCHAR)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
    var_error INTEGER;
...
BEGIN
    ...
    IF par_hkid IS NULL OR CHAR_LENGTH(RTRIM(par_hkid)) != 9 THEN
        BEGIN
            SELECT
                'N'
                INTO par_valid_flag;
...
ALTER PROCEDURE "cpi_pq_validate_hkid" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";
