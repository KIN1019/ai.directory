CREATE OR REPLACE PROCEDURE web_hkpmi_get_hkpmi_by_nokname( INOUT pas_return_code int, IN par_nok_name VARCHAR, IN par_last_name VARCHAR, IN par_last_hkid VARCHAR, IN par_authority_code INTEGER, IN par_hosp_code VARCHAR,INOUT p_refcur refcursor)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
    ...
    SELECT
        ...
        WHERE
        /* --paging modify */
        n.nok_name LIKE par_nok_name AND n.major_nok = 'Y' AND p.patient_name >= par_last_name AND n.patient_key = p.patient_key AND (p.access_code & par_authority_code) > 0;
BEGIN
    /* for search patient list by major nok name only */
    /* if search by major nok hkid, use web_hkpmi_get_hkpmi_by_nokhkid */
...
ALTER PROCEDURE "web_hkpmi_get_hkpmi_by_nokname" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";
