-- DROP PROCEDURE hkpmi.hkpmi_hago_mini_app_check(inout int4, in varchar, inout int4, inout varchar, inout refcursor);

CREATE OR REPLACE PROCEDURE hkpmi.hkpmi_hago_mini_app_check(INOUT pas_return_code integer, IN par_hkid character varying, INOUT par_code integer, INOUT par_status character varying, INOUT p_refcur refcursor)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    ...
    SELECT
        ...
        INTO var_patient_key, var_old_hkid, var_txn_type, var_move_status, var_dob, var_age, var_bcf_date, var_death, var_death_indicator, var_death_date;
    SELECT
        patient_key, dob, death_indicator, death_date
        INTO var_patient_key, var_dob, var_death_indicator, var_death_date
        FROM patient
        WHERE hkid = par_hkid;
    ...
    ELSE
        BEGIN
            /* Deceased patient checking (HAGO-912) */
            IF var_death_indicator IS NOT NULL AND var_death_date IS NOT NULL THEN
                BEGIN
                    IF var_death_indicator = 'ADT' THEN
                        BEGIN
                            ...
                            IF var_bcf_date IS NOT NULL AND var_bcf_date <= - 7 * INTERVAL '1 day' + localtimestamp::TIMESTAMP THEN
                                BEGIN
                                    SELECT
                                        'Y'
                                        INTO var_death;
                ...
                END;
            END IF;
            /* End - Deceased patient checking (HAGO-912) */
...
ALTER PROCEDURE "hkpmi_hago_mini_app_check" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";
