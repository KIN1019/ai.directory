-- DROP PROCEDURE hkpmi.hkpmi_hago_reg_chk_by_age(inout int4, in varchar, in varchar, in varchar, inout int4, inout varchar);

CREATE OR REPLACE PROCEDURE hkpmi.hkpmi_hago_reg_chk_by_age(INOUT pas_return_code integer, IN par_hkid character varying, IN par_input_name character varying, IN par_check_type character varying, INOUT par_code integer, INOUT par_status character varying)
 LANGUAGE plpgsql
AS $procedure$
/* íÑADULTíª or íÑMINORíª */
DECLARE
...
BEGIN
    ...
    IF par_check_type = 'ADULT' AND "var_yrDiff" < 18 THEN
        BEGIN
            SELECT
                2908, 'Not Eligible'
                INTO par_code, par_status;
        ...
        END;
    ELSE
        IF par_check_type = 'MINOR' AND "var_yrDiff" >= 18 THEN
            BEGIN
                SELECT
                    2911, 'Not Eligible'
                    INTO par_code, par_status;
                pas_return_code := 2911;
                RETURN;
            END;
        END IF;
    END IF;
    ...
    SELECT
        ...
        SELECT
            move_episode_indicator.hospital_code, move_episode_indicator.case_no, move_episode_indicator.create_dtm, move_episode_indicator.from_patient_key, move_episode_indicator.to_patient_key, move_episode_indicator.create_user, move_episode_indicator.create_system, move_episode_indicator.move_status, move_episode_indicator.update_dtm, move_episode_indicator.update_user, move_episode_indicator.update_system, move_episode_indicator.info_source_code, move_episode_indicator.reason_code, move_episode_indicator.other_reason
            FROM move_episode_indicator
            WHERE move_status = 'O' AND to_patient_key = var_patient_key LIMIT 1) AS tem;
...
ALTER PROCEDURE "hkpmi_hago_reg_chk_by_age" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";
