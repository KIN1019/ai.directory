-- DROP FUNCTION hkpmi.dw_bp_get_ehr_patient_v2(timestamp, timestamp, int4);

CREATE OR REPLACE FUNCTION hkpmi.dw_bp_get_ehr_patient_v2(par_startdtm timestamp without time zone, par_enddtm timestamp without time zone DEFAULT NULL::timestamp without time zone, par_maxrow integer DEFAULT 0)
...
BEGIN
    ...
    */
    ...
    OPEN p_refcur FOR
    SELECT
        ehr_number, ehr_start_date, ehr_end_date, ehr_flag, pas_hkic, pas_pky, pas_doc_type, pas_doc_no, sys_dtm
...
ALTER FUNCTION "dw_bp_get_ehr_patient_v2" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";
