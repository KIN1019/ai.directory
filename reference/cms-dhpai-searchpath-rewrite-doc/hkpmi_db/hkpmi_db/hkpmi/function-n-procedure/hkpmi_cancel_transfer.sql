...
begin
    ...
    BEGIN
        ...
        BEGIN
            ...
            /* Validate key fields */
            IF NOT ((par_source_system = 'ADT' AND par_txn_type IN ('220', '230', '240', '710')) OR (par_source_system = 'DNL' AND par_txn_type IN ('220', '230', '240', '710'))) THEN
                BEGIN
            ...
            ELSE
                BEGIN
                    IF var_case_discharge_code IS NOT NULL THEN
                        BEGIN
                            /* Case already discharged, cancel transfer is not allowed */
                            SELECT
                                200132
                                INTO var_return_error_code;
                            EXIT return_error;
                    ...
                    IF var_case_movement_count = 1 THEN
                        BEGIN
                            /* no transfer occured, transfer is rejected */
                            SELECT
                                200144
                                INTO var_return_error_code;
                            EXIT return_error;
            ...
            /*
            20040810
            	fall back the rule for checking bed_no
            
            if @case_last_specialty_code is not null and
            	(@case_last_ward_code <> @old_ward_code or
            	@case_last_specialty_code <> @old_specialty_code or
            	(@case_last_ward_class <> @old_ward_class and
            		@case_last_ward_class is not null) or
            	(@case_last_bed_no <> @old_bed_no and
            		@case_last_bed_no is not null)) and
            	@source_system <> "DNL"
            */
            IF (COALESCE(var_case_last_ward_code, '') <> COALESCE(par_old_ward_code, '') OR COALESCE(var_case_last_specialty_code, '') <> COALESCE(par_old_specialty_code, '') OR COALESCE(var_case_last_ward_class, '') <> COALESCE(par_old_ward_class, '') OR COALESCE(var_case_last_bed_no, '') <> COALESCE(par_old_bed_no, '')) AND par_source_system <> 'DNL' THEN
                BEGIN
                    /*
            ...
            WHILE 1 = 1 LOOP
                BEGIN
                    ...
                    /* case information */
                    case_no, patient_type, case_type, ward_code, specialty_code, ward_class, bed_no, old_ward_code, old_specialty_code, old_ward_class, old_bed_no, update_by, update_hospital, source_system, source_system_dtm, upload_status)
                    ...
                    /* case information */
                    par_case_no, var_case_patient_type, var_case_case_type, par_last_ward_code, par_last_specialty_code, par_last_ward_class, par_last_bed_no, par_old_ward_code, par_old_specialty_code, par_old_ward_class, par_old_bed_no, par_update_by, par_hospital_code, par_source_system, par_source_system_dtm, var_upload_status);
                    var_error := 0;
                    EXCEPTION
                        WHEN OTHERS THEN
