-- DROP FUNCTION download.pas_get_pmi_trx_v2(varchar);

CREATE OR REPLACE FUNCTION download.pas_get_pmi_trx_v2(par_last_poll_mark character varying)
...
DECLARE
    ...
    var_poll_row_count INTEGER;
    var_max_poll_mark_allowed TIMESTAMP WITHOUT TIME ZONE;
    var_new_last_poll_mark_in_dt TIMESTAMP WITHOUT TIME ZONE;
BEGIN
    ...
    -- IF DATE_PART('days', var_curr_dt::TIMESTAMP-var_last_poll_mark_in_dt::TIMESTAMP) > 30 THEN
    --     /* --IF DATEDIFF(HH, @last_poll_mark_in_dt, @curr_dt) > 6 */
    --     BEGIN
    --         OPEN p_refcur FOR
    --         SELECT
    --             'Polling date cannot be 30 days ago!';
    --         /* --SELECT 'Polling date cannot be 6 hours ago!' */
    --         RETURN NEXT p_refcur;
    --     END;
    -- END IF;
    IF DATE_PART('epoch', var_curr_dt::TIMESTAMP-var_last_poll_mark_in_dt::TIMESTAMP) / 3600 > 12 THEN
        BEGIN
            OPEN p_refcur FOR
            SELECT
                'Polling date cannot be 12 hours ago!';
            RETURN NEXT p_refcur;
        END;
    END IF;
    SELECT
        0
        INTO var_poll_row_count;
    /* 2014-03-10 Sam - Avoid missing messages - START */
    /* @max_poll_mark_allowed = DATEADD(SECOND, -20, @curr_dt) */
    /* 2012-12-10 Sam - Limit the time of maximum poll mark between current time to 20 seconds */
    /* 2014-03-10 Sam - Avoid missing messages - END */
    /* @max_poll_mark_allowed = @curr_dt */
    /* 2013-01-14 Sam - Enhancement on poll interval */
    <<loop_again>>
    LOOP
        /* 2014-03-10 Sam - Avoid missing messages - START */
        SELECT
            var_curr_dt
            INTO var_max_poll_mark_allowed;
        /* 2014-03-10 Sam - Avoid missing messages - END */
        SELECT
            5 * INTERVAL '1 minute' + var_last_poll_mark_in_dt::TIMESTAMP
            INTO var_new_last_poll_mark_in_dt;

        IF var_new_last_poll_mark_in_dt > var_max_poll_mark_allowed THEN
            SELECT
                var_max_poll_mark_allowed
                INTO var_new_last_poll_mark_in_dt;
        END IF;
        EXIT loop_again;
        SELECT
            ...
            WHERE a.system_dtm > var_last_poll_mark_in_dt AND a.system_dtm <= var_new_last_poll_mark_in_dt AND
        ...
        IF (var_poll_row_count < 1) AND (var_new_last_poll_mark_in_dt < var_max_poll_mark_allowed) THEN
            BEGIN
                SELECT
                    var_new_last_poll_mark_in_dt
                    INTO var_last_poll_mark_in_dt;
    ...
    */
    ...
    SELECT
        ...
        */
        ...
        ORDER BY a."system_dtm" NULLS FIRST, a."hospital_code" NULLS FIRST, a.row_update_datetime NULLS FIRST /* Sam 2010-08-03 Include timestamp to sort 261 (262 + 260) correctly */;
    RETURN NEXT p_refcur;
...
ALTER FUNCTION "pas_get_pmi_trx_v2" OWNER TO "DOWNLOAD_SCHEMA_OWNER_ROLE";