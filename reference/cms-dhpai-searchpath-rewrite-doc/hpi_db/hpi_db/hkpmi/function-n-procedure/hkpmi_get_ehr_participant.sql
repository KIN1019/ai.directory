-- DROP FUNCTION hkpmi_get_ehr_participant(varchar, varchar, varchar, varchar);

CREATE OR REPLACE FUNCTION hkpmi_get_ehr_participant(par_patient_key character varying, par_hkid character varying, par_source_system character varying, par_source_function character varying DEFAULT NULL::character varying)
...
DECLARE
    var_ehr_number VARCHAR(12);
    var_ehr_start_date VARCHAR(10);
    var_ehr_end_date VARCHAR(10);
    var_pas_patient_key VARCHAR(8);
...
BEGIN
    ...
    BEGIN
        ...
        /*
        select @live_run_flag = run_flag
        from ehr_event_conf
        where config_id = 11
        and system_id = 'EHR_CMS_WS'
        ...
        */
        ...
        IF (LTRIM(RTRIM(par_patient_key)) = '') OR (LTRIM(RTRIM(par_patient_key)) = NULL) OR (LTRIM(RTRIM(par_source_system)) = '') OR (LTRIM(RTRIM(par_source_system)) = NULL) OR (LTRIM(RTRIM(par_hkid)) = '') OR (LTRIM(RTRIM(par_hkid)) = NULL) THEN
            BEGIN
                EXIT error;
            END;
        END IF;
                        ...
                        IF (var_ehr_count > 1 AND var_ehr_hkid_count > 0) THEN
                            BEGIN
                                SELECT
                                    ...
                                    ORDER BY sys_dtm DESC NULLS FIRST, ehr_start_date DESC NULLS FIRST
                                    LIMIT 1;
                            END;
                        ELSE
                            BEGIN
        ...
        IF var_ehr_start_date IS NOT NULL AND var_ehr_start_date <> '' THEN
            BEGIN
                SELECT
                    CONCAT(SUBSTRING(var_ehr_start_date, 7, 2), '/', SUBSTRING(var_ehr_start_date, 5, 2), '/', SUBSTRING(var_ehr_start_date, 1, 4))
        ...
        IF var_ehr_end_date IS NOT NULL AND var_ehr_end_date <> '' THEN
            BEGIN
                SELECT
                    CONCAT(SUBSTRING(var_ehr_end_date, 7, 2), '/', SUBSTRING(var_ehr_end_date, 5, 2), '/', SUBSTRING(var_ehr_end_date, 1, 4))
        ...
        IF par_source_system IN ('CMS', 'PMS', 'CDR', 'RIS', 'ePR', 'MCAF', 'eSHR', 'LIS', 'MOE') THEN
            BEGIN
                ...
                SELECT
                    ...
                    SUBSTRING(filler, 1, 1), other_doc_no
                ...
                IF var_patient_name_len > 0 THEN
                    BEGIN
                        ...
                        IF var_comma_index <> 0 THEN
                            BEGIN
                                SELECT
                                    RTRIM(LTRIM(SUBSTRING(var_patient_name, 1, var_comma_index - 1)))
                                    INTO var_last_name;

                                IF var_comma_index < LENGTH(var_patient_name) THEN
                                    BEGIN
                                        SELECT
                                            RTRIM(LTRIM(SUBSTRING(var_patient_name, var_comma_index + 1, LENGTH(var_patient_name) - var_comma_index)))
                                            INTO var_first_name;
                                    END;
                                END IF;
                            END;
        ...
        ELSE
            ...
            BEGIN
                ...
                IF var_ehr_dob IS NOT NULL AND var_ehr_dob <> '' THEN
                    BEGIN
                        SELECT
                            CONCAT(SUBSTRING(var_ehr_dob, 7, 2), '/', SUBSTRING(var_ehr_dob, 5, 2), '/', SUBSTRING(var_ehr_dob, 1, 4))
                            INTO var_ehr_dob;
                    END;
                END IF;
                ...
                IF var_pas_dob IS NOT NULL AND var_pas_dob <> '' THEN
                    BEGIN
                        SELECT
                            CONCAT(SUBSTRING(var_pas_dob, 7, 2), '/', SUBSTRING(var_pas_dob, 5, 2), '/', SUBSTRING(var_pas_dob, 1, 4))
                            INTO var_pas_dob;
                    END;
                END IF;
                OPEN p_refcur FOR
...
ALTER FUNCTION "hkpmi_get_ehr_participant" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";