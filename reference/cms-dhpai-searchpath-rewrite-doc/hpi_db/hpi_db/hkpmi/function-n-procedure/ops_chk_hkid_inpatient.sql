...
BEGIN
    ...
    IF EXISTS (SELECT
        1
        FROM pmi_case
        WHERE patient_key = par_patient_key AND case_type = 'I' AND adm_dtm <= par_slot_datetime AND (discharge_dtm > par_slot_datetime OR discharge_dtm IS NULL)) THEN
        BEGIN
            SELECT
