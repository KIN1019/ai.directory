-- DROP PROCEDURE hkpmi_check_merge(inout int4, in varchar, in varchar, in varchar, in int4, in varchar);

CREATE OR REPLACE PROCEDURE hkpmi_check_merge(INOUT pas_return_code integer, IN par_hospital_code character varying, IN par_from_hkid character varying, IN par_to_hkid character varying, IN par_error integer, IN par_check_type character varying DEFAULT 'MERGE'::character varying)
 LANGUAGE plpgsql
AS $procedure$
BEGIN
    <<program_return>>
