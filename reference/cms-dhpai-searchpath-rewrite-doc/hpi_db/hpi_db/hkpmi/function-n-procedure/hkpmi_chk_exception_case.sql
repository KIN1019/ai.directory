-- DROP PROCEDURE hkpmi_chk_exception_case(inout int4, in varchar, in varchar, inout varchar);

CREATE OR REPLACE PROCEDURE hkpmi_chk_exception_case(INOUT pas_return_code integer, IN par_hospital_cde character varying, IN par_case_no character varying, INOUT par_exception_flag character varying)
 LANGUAGE plpgsql
AS $procedure$
BEGIN
