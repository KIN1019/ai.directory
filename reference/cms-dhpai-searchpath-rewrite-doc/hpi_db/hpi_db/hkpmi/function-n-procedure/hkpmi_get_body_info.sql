-- DROP PROCEDURE hkpmi_get_body_info(inout int4, in varchar, in varchar, inout varchar, inout varchar, inout varchar, inout varchar, inout varchar, inout varchar, inout varchar, inout varchar, inout varchar, inout timestamp, inout varchar, inout timestamp, inout varchar, inout int4, inout varchar);

CREATE OR REPLACE PROCEDURE hkpmi_get_body_info(INOUT pas_return_code integer, IN par_hospital_code character varying, IN par_case_no character varying, INOUT par_hkid character varying, INOUT par_patient_name character varying, INOUT par_sex character varying, INOUT par_cccode1 character varying, INOUT par_cccode2 character varying, INOUT par_cccode3 character varying, INOUT par_cccode4 character varying, INOUT par_cccode5 character varying, INOUT par_cccode6 character varying, INOUT par_dob timestamp without time zone, INOUT par_exact_dob_flag character varying, INOUT par_death_date timestamp without time zone, INOUT par_body_category character varying, INOUT par_retcode integer, INOUT par_error_msg character varying)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
        ...
        ELSE
            IF par_hkid IS NOT NULL THEN
                BEGIN
                    ...
                    */
                    SELECT
                        hkid, patient_name, sex, cccode1, cccode2, cccode3, cccode4, cccode5, cccode6, dob, exact_dob_flag, death_date, SUBSTRING(filler, 2, 1)
                        INTO par_hkid, par_patient_name, par_sex, par_cccode1, par_cccode2, par_cccode3, par_cccode4, par_cccode5, par_cccode6, par_dob, par_exact_dob_flag, par_death_date, par_body_category
                        FROM patient
                        WHERE hkid = par_hkid;
                    GET DIAGNOSTICS sql$rowcount = ROW_COUNT;
...
ALTER PROCEDURE "hkpmi_get_body_info" OWNER TO "HKPMI_SCHEMA_OWNER_ROLE";