-- DROP FUNCTION hkpmi_r_case_list(varchar);

CREATE OR REPLACE FUNCTION hkpmi_r_case_list(par_hkid character varying)
 RETURNS TABLE(hospital_code character varying, case_no character varying, adt1 text, adt2 text, adt3 text, adt4 text, destination_code character varying, last_specialty_code character varying, last_ward_code character varying)
 LANGUAGE plpgsql
AS $function$
