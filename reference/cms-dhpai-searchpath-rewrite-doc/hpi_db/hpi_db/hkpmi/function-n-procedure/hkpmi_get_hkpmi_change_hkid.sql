-- DROP FUNCTION hkpmi_get_hkpmi_change_hkid(varchar);

CREATE OR REPLACE FUNCTION hkpmi_get_hkpmi_change_hkid(par_from_hkid character varying)
 RETURNS TABLE(res_hkid character varying, res_patient_name character varying, res_sex character varying, res_dob timestamp without time zone, res_hospital_code character varying, res_update_by character varying, res_system_dtm timestamp without time zone, res_d integer)
 LANGUAGE plpgsql
AS $function$
