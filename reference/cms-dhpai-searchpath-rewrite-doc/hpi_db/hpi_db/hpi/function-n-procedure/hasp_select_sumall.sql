-- DROP FUNCTION hpi.hasp_select_sumall(varchar, timestamp, timestamp, varchar, varchar, varchar);

CREATE OR REPLACE FUNCTION hpi.hasp_select_sumall(par_hosp_code character varying, par_trans_sys_fdatetime timestamp without time zone, par_trans_sys_tdatetime timestamp without time zone, par_trans_ns_code character varying, par_from_time character varying, par_to_time character varying)
    ...
    */
    ...
    select t1.System_datetime,
    	      ...
    	      (t1.Transaction_type in ('140','160','170','700','710') or
                 ...
                 ((t1.From_ward_code like @TRANS_NS_CODE) or
                  (t1.To_ward_code  like @TRANS_NS_CODE)) and
                  (datepart(hh, t1.System_datetime) =
                   convert(smallint, t2.temp_hh))
         union
            select t1.System_datetime, t2.temp_hh,
                  (select DT_code from DT_transaction_type
            ...
            convert(char(30),convert(datetime,@TRANS_SYS_TDATETIME),103),
             ...
             where
    //              (t1.Transaction_type in ('140','700','710') or //
    ...
    */
    ...
    SELECT
        t1.System_datetime, t2.temp_hh, (SELECT
            ...
            WHERE ADT_code = t1.Transaction_type), t1.From_ward_code, t1.To_ward_code, d1.Short_description, par_TRANS_SYS_TDATETIME, par_TRANS_SYS_FDATETIME, par_TRANS_NS_CODE, to_char(par_TRANS_SYS_TDATETIME::TIMESTAMP WITHOUT TIME ZONE, 'DD/MM/YYYY'), t1.System_datetime, par_FROM_TIME, par_TO_TIME
        FROM Transaction_log AS t1, TEMP_TRANS_SUMMARY AS t2, Discharge_type AS d1
        WHERE t1.Hospital_code = par_hosp_code AND SUBSTRING(t1.Transaction_type, 3, 1) = d1.Discharge_code AND (t1.Transaction_type IN ('140', '160', '170', '700', '710', '230', '240') OR t1.Transaction_type LIKE '13%' OR t1.Transaction_type LIKE '21%' OR t1.Transaction_type LIKE '33%' OR t1.Transaction_type LIKE '35%') AND (t1.System_datetime >= par_TRANS_SYS_FDATETIME) AND (t1.System_datetime <= par_TRANS_SYS_TDATETIME) AND ((t1.From_ward_code LIKE par_TRANS_NS_CODE) OR (t1.To_ward_code LIKE par_TRANS_NS_CODE)) AND (date_part('hour', t1.System_datetime::TIMESTAMP) = CAST (t2.temp_hh AS SMALLINT))
    UNION
    SELECT
    ...
    SELECT
        ...
        FROM Transaction_log AS t1, TEMP_TRANS_SUMMARY AS t2, Discharge_type AS d1
        WHERE t1.Hospital_code = par_hosp_code AND t1.Transaction_type = '220' AND (t1.System_datetime >= par_TRANS_SYS_FDATETIME) AND (t1.System_datetime <= par_TRANS_SYS_TDATETIME) AND ((t1.From_ward_code LIKE par_TRANS_NS_CODE) OR (t1.To_ward_code LIKE par_TRANS_NS_CODE)) AND (date_part('hour', t1.System_datetime::TIMESTAMP) = CAST (t2.temp_hh AS SMALLINT))
    UNION
    SELECT
    '1900-01-01 00:00:00.000'::TIMESTAMP, 
    temp_hh,
    ''::VARCHAR,   
    par_TRANS_NS_CODE,
    par_TRANS_NS_CODE,
    '',
    par_TRANS_SYS_TDATETIME,
    par_TRANS_SYS_FDATETIME,
    par_TRANS_NS_CODE,
    to_char(par_TRANS_SYS_TDATETIME, 'DD/MM/YYYY'),
    par_TRANS_SYS_TDATETIME,
    par_FROM_TIME,
    par_TO_TIME
FROM TEMP_TRANS_SUMMARY
WHERE (CAST(temp_hh AS SMALLINT) <= date_part('hour', par_TRANS_SYS_TDATETIME))
  AND (CAST(temp_hh AS SMALLINT) >= date_part('hour', par_TRANS_SYS_FDATETIME))
...
;ALTER FUNCTION "hasp_select_sumall" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
