-- DROP FUNCTION hpi."fn_tU_User_group_detail"();

CREATE OR REPLACE FUNCTION hpi."fn_tU_User_group_detail"()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
/* UPDATE trigger on User_group_detail */
DECLARE
    var_numrows INTEGER;
    var_nullcnt INTEGER;
    var_validcnt INTEGER;
    "var_insGroup_ID" CHAR(10);
    "var_insFunc_option" INTEGER;
...
BEGIN
    ...
    BEGIN
        ...
        /* User_group may have User_group_detail ON CHILD UPDATE RESTRICT */
        IF update$Group_ID THEN
            BEGIN
                ...
                IF var_validcnt + var_nullcnt != var_numrows THEN
                    BEGIN
                        RAISE EXCEPTION '% % % ', 'UPDATE', 'User_group_detail', 'User_group' USING ERRCODE := '200012';
                        EXIT error;
                    END;
                END IF;
            END;
        END IF;
        /* Func_table join with User_group_detail ON CHILD UPDATE RESTRICT */
        IF update$Func_ID THEN
            BEGIN
                ...
                IF var_validcnt + var_nullcnt != var_numrows THEN
                    BEGIN
                        RAISE EXCEPTION '% % % ', 'UPDATE', 'User_group_detail', 'Func_table' USING ERRCODE := '200012';
                        EXIT error;
                    END;
                END IF;
            END;
...
;ALTER FUNCTION "fn_tU_User_group_detail" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
