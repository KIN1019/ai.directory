CREATE OR REPLACE PROCEDURE opas_get_cuid(INOUT pas_return_code int, IN par_server_name VARCHAR, IN par_hkid VARCHAR, IN par_name VARCHAR, INOUT par_cuid_exist VARCHAR, INOUT par_cuid_name VARCHAR, INOUT par_cuid VARCHAR, INOUT par_raiserror_text VARCHAR)
AS 
$BODY$
DECLARE
...
BEGIN
    ...
    */ /* ----begran tran ... */
    SELECT
        CONCAT(RTRIM(par_server_name), '.proc_cuid_get_cuid')
        INTO var_rpc_call;
		
    perform public.dblink_connect('rpc_server'::text, var_rpc_call);
	select * from public.dblink('rpc_server'::text, 'call proc_cuid_get_cuid'
	|| '('
	|| case when par_hkid is null then 'null::bpchar' else concat('''', par_hkid, '''::bpchar') end || ','
	|| case when par_name is null then 'null::bpchar' else concat('''', par_name, '''::bpchar') end || ','
	|| case when par_cuid_exist is null then 'null::bpchar' else concat('''', par_cuid_exist, '''::bpchar') end || ','
	|| case when par_cuid_name is null then 'null::bpchar' else concat('''', par_cuid_name, '''::bpchar') end || ','
	...
	as t1(par_cuid_exist, par_cuid_name, par_cuid, par_raiserror_text, var_return_code) 
	into par_cuid_exist, par_cuid_name, par_cuid, par_raiserror_text, var_return_code;
