CREATE OR REPLACE PROCEDURE hasp_get_ce_stat(INOUT pas_return_code int, IN par_input_from_date TIMESTAMP WITHOUT TIME ZONE, IN par_input_to_date TIMESTAMP WITHOUT TIME ZONE, IN par_hospital_code VARCHAR)
AS 
$BODY$
DECLARE
...
BEGIN
    ...
    WHILE var_date <= par_input_to_date LOOP
        ...
        END IF;
        SELECT
            CONCAT(SUBSTRING(to_char(var_date, 'YYYMMDD'), 1, 8),
				   SUBSTRING(CONCAT(var_hosp, REPEAT(' ', 3)), 1, 3),
				   SUBSTRING(CONCAT(CAST (var_ae_att AS VARCHAR(8)), REPEAT(' ', 8)), 1, 8),
				   SUBSTRING(CONCAT(CAST (var_prev_ae_att_per AS VARCHAR(10)), REPEAT(' ', 10)), 1, 10),
				   SUBSTRING(CONCAT(CAST (var_ae_adm AS VARCHAR(8)), REPEAT(' ', 8)), 1, 8),
				   SUBSTRING(CONCAT(CAST (var_prev_ae_adm_per AS VARCHAR(10)), REPEAT(' ', 10)), 1, 10),
				   SUBSTRING(CONCAT(CAST (var_adm AS VARCHAR(8)), REPEAT(' ', 8)), 1, 8),
				   SUBSTRING(CONCAT(CAST (var_prev_adm_per AS VARCHAR(10)), REPEAT(' ', 10)), 1, 10),
				   SUBSTRING(CONCAT(CAST (var_ae_adm_per AS VARCHAR(10)), REPEAT(' ', 10)), 1, 10),
				   SUBSTRING(CONCAT(CAST (var_occ_rate AS VARCHAR(10)), REPEAT(' ', 10)), 1, 10),
				   SUBSTRING(CONCAT(CAST (var_vbd AS VARCHAR(8)), REPEAT(' ', 8)), 1, 8),
				   SUBSTRING(CONCAT(CAST (var_prev_vbd_per AS VARCHAR(10)), REPEAT(' ', 10)), 1, 10),
				   SUBSTRING(CONCAT(CAST (var_ebd AS VARCHAR(8)), REPEAT(' ', 8)), 1, 8),
				   SUBSTRING(CONCAT(CAST (var_prev_ebd_per AS VARCHAR(10)), REPEAT(' ', 10)), 1, 10),
				   SUBSTRING(CONCAT(var_cluster, REPEAT(' ', 5)), 1, 5))
            INTO var_string;
