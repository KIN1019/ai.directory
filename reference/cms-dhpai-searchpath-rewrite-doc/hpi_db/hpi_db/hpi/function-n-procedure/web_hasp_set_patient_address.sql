-- DROP PROCEDURE hpi.web_hasp_set_patient_address(inout int4, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, inout int4, inout varchar);

CREATE OR REPLACE PROCEDURE hpi.web_hasp_set_patient_address(INOUT pas_return_code integer, IN par_hkid character varying, IN par_address_type character varying DEFAULT 'C'::character varying, IN par_building character varying DEFAULT NULL::character varying, IN par_room character varying DEFAULT NULL::character varying, IN par_floor character varying DEFAULT NULL::character varying, IN par_block character varying DEFAULT NULL::character varying, IN par_district_code character varying DEFAULT NULL::character varying, IN par_hospital_code character varying DEFAULT NULL::character varying, IN par_source_system character varying DEFAULT NULL::character varying, IN par_user_id character varying DEFAULT NULL::character varying, INOUT par_pas_return_code integer DEFAULT NULL::integer, INOUT par_return_message character varying DEFAULT NULL::character varying)
 LANGUAGE plpgsql
AS $procedure$
...
begin
        ...
        */
     CALL cpi_set_patient_address(var_retcode,par_hkid, par_address_type, par_building, par_room, par_floor, par_block, par_district_code, par_hospital_code, par_source_system, par_user_id, par_pas_return_code, par_return_message);
...
--	    CALL cpi_get_rpc_server(pas_return_code, 'HKPMI_SERVER', var_hkpmi_srvr);
	...
	--        				@return_code out,@return_message out
	   	 ...
	   	 var_db_sql := 'call ' || var_rpc_call || '('
		        ...
		        || case when par_pas_return_code is null then '0' else 0 end || ','
		        || case when par_return_message is null then 'null::bpchar' else concat('''', par_return_message, '''::bpchar') end || ');';
		           ...
		           select * from dblink('HKPMI_SERVER', var_db_sql)  as t1(
						        ...
						        par_floor character,
						        par_block character,
						        par_district_code character,
						        par_pas_return_code integer,
...
;ALTER PROCEDURE "web_hasp_set_patient_address" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
