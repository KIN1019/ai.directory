-- DROP FUNCTION hpi."fn_tI_Diagnosis"();

CREATE OR REPLACE FUNCTION hpi."fn_tI_Diagnosis"()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
/* INSERT trigger on Diagnosis */
DECLARE
    var_numrows INTEGER;
    var_nullcnt INTEGER;
...
BEGIN
    ...
    BEGIN
        ...
        IF var_numrows = 0 THEN
            RETURN NULL;
        END IF;
        /* Case may have Diagnosis ON CHILD INSERT RESTRICT */
        IF update$Case_no THEN
            BEGIN
                ...
                IF var_validcnt + var_nullcnt != var_numrows THEN
                    BEGIN
                        RAISE EXCEPTION '% % % ', 'INSERT', 'Diagnosis', 'cpi_case' USING ERRCODE := '200012';
                        EXIT error;
                    END;
                END IF;
            END;
...
;ALTER FUNCTION "fn_tI_Diagnosis" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
