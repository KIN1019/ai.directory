-- DROP FUNCTION "fn_tU_cpi_movement"();

CREATE OR REPLACE FUNCTION "fn_tU_cpi_movement"()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
/* UPDATE trigger on cpi_movement */
DECLARE
    var_numrows INTEGER;
    var_nullcnt INTEGER;
...
BEGIN
    ...
    BEGIN
        ...
        IF var_numrows > 1 THEN
            BEGIN
                RAISE EXCEPTION '% % ', 'UPDATE', 'cpi_movement' USING ERRCODE := '200013';
                EXIT error;
            END;
        END IF;
        SELECT
            ...
            BEGIN
                ...
                IF "var_insMovement_count" > 1 AND NOT EXISTS (SELECT
                    *
                    ...
                    BEGIN
                        RAISE EXCEPTION USING ERRCODE := '200017';
                        EXIT error;
                    END;
                END IF;
            END;
        END IF;
        /* ward join with cpi_movement ON CHILD UPDATE RESTRICT */
        IF update$ward_code AND var_case_type != 'O' THEN
            BEGIN
                SELECT
                    0
                    INTO var_nullcnt;
                ...
                */
                ...
                IF var_validcnt + var_nullcnt != var_numrows THEN
                    BEGIN
                        RAISE EXCEPTION '% % % ', 'UPDATE', 'cpi_movement', 'ward' USING ERRCODE := '200012';
                        EXIT error;
                    END;
                END IF;
            END;
        END IF;
        /* cpi_case Must have one cpi_movement ON CHILD UPDATE RESTRICT */
        IF update$case_no THEN
            BEGIN
                ...
                IF var_validcnt + var_nullcnt != var_numrows THEN
                    BEGIN
                        RAISE EXCEPTION '% % % ', 'UPDATE', 'cpi_movement', 'cpi_case' USING ERRCODE := '200012';
                        EXIT error;
                    END;
                END IF;
            END;
        END IF;
        /* Bed join with cpi_movement ON CHILD UPDATE RESTRICT */
        IF update$bed_no THEN
            BEGIN
                ...
                SELECT
                    ...
                    INTO var_validcnt
                    FROM inserted, Bed
                    WHERE inserted.ward_code = Bed.Ward_code AND inserted.bed_no = Bed.Bed_no AND Bed.Status = 'V' AND inserted.hospital_code = Bed.Hospital_code;
                SELECT
                    COUNT(*)
                    INTO var_nullcnt
                    FROM inserted
                ...
                IF var_validcnt + var_nullcnt != var_numrows THEN
                    BEGIN
                        RAISE EXCEPTION '% % % ', 'UPDATE', 'cpi_movement', 'Bed' USING ERRCODE := '200012';
                        EXIT error;
                    END;
                END IF;
            END;
        END IF;
        /* ip_specialty join with cpi_movement ON CHILD UPDATE RESTRICT */
                ...
                */
                SELECT
                    ...
                    WHERE ip_specialty.effective_date = (SELECT
                        ...
                        FROM ip_specialty, inserted
                        WHERE ip_specialty.effective_date <= inserted.movement_dtm AND ip_specialty.specialty_code = inserted.specialty AND ip_specialty.hospital_code = inserted.hospital_code) AND inserted.specialty = ip_specialty.specialty_code AND ip_specialty.active_status = 'A' AND ip_specialty.hospital_code = inserted.hospital_code;

                IF var_validcnt + var_nullcnt != var_numrows THEN
                    BEGIN
                        RAISE EXCEPTION '% % % ', 'UPDATE', 'cpi_movement', 'ip_specialty' USING ERRCODE := '200012';
                        EXIT error;
                    END;
                END IF;
            END;
...
;ALTER FUNCTION "fn_tU_cpi_movement" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
