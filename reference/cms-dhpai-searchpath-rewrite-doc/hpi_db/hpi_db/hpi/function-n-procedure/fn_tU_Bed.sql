-- DROP FUNCTION hpi."fn_tU_Bed"();

CREATE OR REPLACE FUNCTION hpi."fn_tU_Bed"()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
/* UPDATE trigger on Bed */
DECLARE
    var_numrows INTEGER;
    var_nullcnt INTEGER;
    var_validcnt INTEGER;
    "var_insWard_code" CHAR(4);
    "var_insBed_no" CHAR(5);
...
BEGIN
    ...
    BEGIN
        ...
        IF var_numrows = 0 THEN
            RETURN NULL;
        END IF;
        /* Ward may have Bed ON CHILD UPDATE RESTRICT */
            ...
            BEGIN
                ...
                */
                ...
                IF var_validcnt + var_nullcnt != var_numrows THEN
                    BEGIN
                        RAISE EXCEPTION '% % % ', 'UPDATE', 'Bed', 'Ward' USING ERRCODE := '200012';
                        EXIT error;
                    END;
                END IF;
            END;
...
;ALTER FUNCTION "fn_tU_Bed" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
