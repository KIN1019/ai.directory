-- DROP FUNCTION hpi.web_cpi_address_search(varchar, varchar, varchar, varchar, varchar, int4);

CREATE OR REPLACE FUNCTION hpi.web_cpi_address_search(par_region character varying, par_street_in character varying, par_build_land_in character varying, par_location_in character varying, par_location_all character varying, par_num_of_rows integer)
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
           ...
           RAISE NOTICE 'par_location_all=%', par_location_all;
            ...
            */
            ...
            IF NOT (par_location_all IS NULL) THEN
                BEGIN
                    /* --Use long key */
	                RAISE NOTICE 'par_location_all=% var_long_key=%', par_location_all, var_long_key;
                    CALL cpi_address_build_long_key(pas_return_code, par_location_all, var_long_key);
                   RAISE NOTICE 'cpi_address_build_long_key var_long_key=%', var_long_key;
                    /* --print @long_key */
                    /* For OAH */
                    ...
                    ELSE
                        SELECT
                            ...
                            INTO var_long_key;
                    END IF;
                    RAISE NOTICE 'var_long_key=%,par_region=%', var_long_key,par_region;
					...
					SELECT DISTINCT
					    record_id, record_type, 9
					    FROM address_key_new AS a 
					    WHERE long_key LIKE var_long_key AND record_type IN ('L', 'V') AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
					...
					SELECT DISTINCT
					    record_id, record_type, 7
					    FROM address_key_new AS a 
					    WHERE long_key LIKE var_long_key AND record_type IN ('U', 'O', 'T', 'A', 'G') AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
                    ...
                    */
                    ...
                    /* --print @short_key */
                   RAISE NOTICE 'var_short_key=%', var_short_key;
                    SELECT
					...
					SELECT DISTINCT
					    record_id, record_type, 6
					    FROM address_key_new AS a 
					    WHERE short_key LIKE var_short_key AND record_type IN ('L', 'V') AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
					...
					SELECT DISTINCT
					    record_id, record_type, 5
					    FROM address_key_new AS a 
					    WHERE short_key LIKE var_short_key AND record_type = 'B' AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
					...
					SELECT DISTINCT
					    record_id, record_type, 4
					    FROM address_key_new AS a 
					    WHERE short_key LIKE var_short_key AND record_type IN ('S', 'I') AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
					...
					SELECT DISTINCT
					    record_id, record_type, 4
					    FROM address_key_new AS a 
					    WHERE short_key LIKE var_short_key AND record_type IN ('U', 'O', 'T', 'A', 'G') AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
                    /*
                    ...
                    */
                    /* Use soundex key */
                   RAISE NOTICE 'par_location_all=% var_soundex_key=%', par_location_all, var_soundex_key;
					...
					SELECT DISTINCT
					    record_id, record_type, 3
					    FROM address_key_new AS a 
					    WHERE soundex_key = var_soundex_key AND record_type IN ('L', 'V') AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
					...
					SELECT DISTINCT
					    record_id, record_type, 2
					    FROM address_key_new AS a 
					    WHERE soundex_key = var_soundex_key AND record_type = 'B' AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
					...
					SELECT DISTINCT
					    record_id, record_type, 1
					    FROM address_key_new AS a 
					    WHERE soundex_key = var_soundex_key AND record_type IN ('S', 'I') AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
					...
					SELECT DISTINCT
					    record_id, record_type, 1
					    FROM address_key_new AS a 
					    WHERE soundex_key = var_soundex_key AND record_type IN ('U', 'O', 'T', 'A', 'G') AND region LIKE par_region 
					    AND NOT EXISTS (
					    	SELECT 1 FROM t$temp_all ta WHERE ta.record_id = a.record_id
						);
            ...
            */
            IF par_build_land_in IS NOT NULL THEN
                BEGIN
                   ...
                   RAISE NOTICE 'var_long_key => [%]',var_long_key;
                    SELECT
                        CONCAT(RTRIM(var_long_key), '%')
                        INTO var_long_key;
                    /*
        ...
        ELSE
            BEGIN
				...
				SELECT 
				    a.record_id, b."search_type"
				FROM address_key_new AS a
				JOIN t$temp_final AS b
				    ON a.short_key = CAST(b."record_id" AS CHAR(7))
				WHERE a.record_type = 'E'
				  AND NOT EXISTS (
				      SELECT 1 FROM t$temp_final tf WHERE tf.record_id = a.record_id
				  );
            END;
        END IF;
        /* select count(*) from #temp_final */
    ...
    */
    /* Added by Ranger 2012-10-31 limit the rowcount */
    SELECT
        CONCAT('select record_id, address_eng, address_chi, district_code from t$temp_result', ' order by search_type desc, district_code collate "C" asc NULLS FIRST ,', ' estate_eng collate "C" asc NULLS FIRST  , bldg_eng collate "C" asc NULLS FIRST  , street_eng collate "C" asc NULLS FIRST  , house_no collate "C" asc NULLS FIRST limit ',
        CASE CAST (par_num_of_rows AS VARCHAR)
            WHEN '' THEN '10'
            ELSE CAST (par_num_of_rows AS VARCHAR)
        END)
        INTO var_v_statement;
    RAISE NOTICE '[web_cpi_address_search:1023]sql=>%', var_v_statement;
    /*
    ...
    */
    DROP TABLE t$temp_all;
    DROP TABLE t$temp_final;
    DROP TABLE t$temp_building;
    DROP TABLE t$temp_location;
    DROP TABLE t$temp_street;
...
;ALTER FUNCTION "web_cpi_address_search" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
