-- DROP PROCEDURE hpi.hasp_cancel_bed_assignment(inout int4, in varchar, in varchar, in varchar, in varchar, in timestamp);

CREATE OR REPLACE PROCEDURE hpi.hasp_cancel_bed_assignment(INOUT pas_return_code integer, IN par_hosp_code character varying, IN par_t_case_no character varying, IN par_t_ns_code character varying, IN par_t_user_id character varying, IN par_t_case_timestamp timestamp without time zone)
 LANGUAGE plpgsql
AS $procedure$
/* -------------------------------------------------------------- */
...
/* O.S.                : AIX 3.2.5 */
/* Function            : To perform cancellation of bed */
/* assignment */
/* AIX file name       : sp_cba.sql */
/* -------------------------------------------------------------- */
DECLARE
    var_error INTEGER;
    ...
    var_tx_cancel_bed_assignment VARCHAR(6);
    var_occupied_bed_status VARCHAR(2);
    var_vacant_bed_status VARCHAR(2);
    var_bed_assignment VARCHAR(2);
    sql$rowcount BIGINT;
BEGIN
    /* declare variables */
    SELECT
        '710'
        INTO var_tx_cancel_bed_assignment;
    SELECT
        'O'
    ...
    */
    ...
    BEGIN
        ...
        IF var_movement_type != var_bed_assignment THEN
            BEGIN
                /* raiserror 102013 Last transaction is not bed assignment */
                /* return 102013 */
                RAISE EXCEPTION 'Last transaction is not bed assignment' USING ERRCODE := '40008';
        ...
        IF var_rowcount != 1 THEN
            ...
            END;
        END IF;
        CALL hasp_insert_transaction_log(var_return_code, par_hosp_code, var_system_datetime, par_T_CASE_NO, par_T_NS_CODE, var_treatment_location, var_ward_class, var_bed_no, var_specialty_code, NULL, NULL, NULL, NULL, NULL, var_movement_datetime, var_tx_cancel_bed_assignment, NULL, par_T_USER_ID, NULL, var_movement_system_datetime);
        ...
        IF var_return_code != 0 THEN
            ...
            END;
        END IF;
        CALL hasp_insert_event_log(var_return_code, par_hosp_code, var_system_datetime, var_tx_cancel_bed_assignment, var_hkid, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, par_T_CASE_NO, var_admission_datetime, var_source_indicator, var_source_code, var_pay_code, var_discharge_code, var_movement_datetime, var_destination_code, var_case_type, var_case_movement_count, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, par_T_NS_CODE, var_specialty_code, NULL, var_ward_class, NULL, NULL, NULL, NULL, var_ward_class, var_ward_code, var_specialty_code, var_bed_no, par_T_USER_ID, NULL, var_old_doctor_code, NULL, NULL);
        ...
        IF var_return_code != 0 THEN
            ...
            END;
        END IF;
        CALL cpi_cancel_transfer(var_return_code, par_hosp_code, par_T_CASE_NO, var_hkid, var_current_ward_code, var_current_ward_class, var_current_bed_no, var_current_specialty_code, var_current_doctor_code, par_T_NS_CODE, var_ward_class, var_bed_no, var_specialty_code, var_old_doctor_code, var_movement_datetime, var_case_type, var_tx_cancel_bed_assignment, var_system_datetime, par_T_USER_ID, 'LRRDT');
...
;ALTER PROCEDURE "hasp_cancel_bed_assignment" OWNER TO "HPI_SCHEMA_OWNER_ROLE";