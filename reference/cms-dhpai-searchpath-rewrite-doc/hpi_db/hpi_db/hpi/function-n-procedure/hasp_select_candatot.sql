-- DROP FUNCTION hpi.hasp_select_candatot(varchar, timestamp, timestamp, varchar);

CREATE OR REPLACE FUNCTION hpi.hasp_select_candatot(par_hosp_code character varying, par_trans_sys_fdatetime timestamp without time zone, par_trans_sys_tdatetime timestamp without time zone, par_trans_ns_code character varying)
...
BEGIN
    ...
    */
    ...
    SELECT
        ...
        FROM hpi.Transaction_log, hpi.cpi_patient AS p, hpi.Case_view AS c, hpi.DT_transaction_type
...
;ALTER FUNCTION "hasp_select_candatot" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
