CREATE OR REPLACE FUNCTION pass_get_ward_patient(IN par_hospital VARCHAR, IN par_ward_list VARCHAR, IN par_spec_list VARCHAR DEFAULT null)
    ...
    IF par_spec_list IS NOT NULL THEN
        BEGIN
           open p_refcur for select rtrim(w.ward_code) as ward_code, rtrim(w.bed_no) as bed_no,
				...
				else null end) as admission_dtm, rtrim(c.case_no), rtrim(w.specialty_code),
			...
			from cpi_patient p
				...
				INNER JOIN t$temp_ward_list AS l on w.ward_code = l.ward_code
				LEFT JOIN cpi_ae_case_detail AS ab on c.case_no = ab.case_no and c.hospital_code = ab.hospital_code
			...
			and w.specialty_code in (select spec_code from t$temp_spec_list)
