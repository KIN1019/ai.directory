-- DROP FUNCTION hpi.cpi_address_search(varchar, varchar, varchar, varchar, varchar);

CREATE OR REPLACE FUNCTION hpi.cpi_address_search(par_region character varying, par_street_in character varying, par_build_land_in character varying, par_location_in character varying, par_location_all character varying)
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            CALL hpi.cpi_address_phonetic_word(var_return_code,par_build_land_in, par_build_land_in);
            CALL hpi.cpi_address_phonetic_word(var_return_code,par_street_in, par_street_in);
            CALL hpi.cpi_address_phonetic_word(var_return_code,par_location_in, par_location_in);
            CALL hpi.cpi_address_phonetic_word(var_return_code,par_location_all, par_location_all);
            CALL hpi.cpi_address_cut_type(var_return_code,par_build_land_in, par_build_land_in);
            CALL hpi.cpi_address_cut_type(var_return_code,par_street_in, par_street_in);
            CALL hpi.cpi_address_cut_type(var_return_code,par_location_in, par_location_in);
            /*
            ...
            */
            CALL hpi.cpi_address_cut_head(var_return_code,par_location_all, par_location_all);
            /*
            print @build_land_in
            select '*'+ @street_in+'*'
            select '*'+@location_in+'*'
            ...
            */
            ...
            IF NOT (par_location_all IS NULL) THEN
                BEGIN
                    /* --Use long key */
                    CALL hpi.cpi_address_build_long_key(var_return_code,par_location_all, var_long_key);
                    /* --print @long_key */
                    /* For OAH */
                    ...
                    */
                    /* Use short key */
                    CALL hpi.cpi_address_build_short_key(var_return_code,par_location_all, var_short_key);
                    /* --print @short_key */
                    SELECT
                    ...
                    */
                    /* Use soundex key */
                    CALL hpi.cpi_address_build_soundex_key(var_return_code,par_location_all, var_soundex_key);
                    /* --print @soundex_key */
                    INSERT INTO t$temp_all
                    SELECT DISTINCT
            ...
            */
            IF NOT (par_location_in IS NULL) THEN
                BEGIN
                    /* Use long key */
                    CALL hpi.cpi_address_build_long_key(var_return_code,par_location_in, var_long_key);
                    SELECT
                        CONCAT(RTRIM(var_long_key), '%')
                        INTO var_long_key;
                    ...
                    */
                    /* Use short key */
                    CALL hpi.cpi_address_build_short_key(var_return_code,par_location_in, var_short_key);
                    SELECT
                        CONCAT(LTRIM(RTRIM(var_short_key)), '%')
                        INTO var_short_key;
                    INSERT INTO t$temp_location
                    ...
                    */
                    /* Use soundex key */
                    CALL hpi.cpi_address_build_soundex_key(var_return_code,par_location_in, var_soundex_key);
                    INSERT INTO t$temp_location
                    SELECT DISTINCT
                        record_id, record_type, 16
                        FROM address_key_new
            ...
            */
            IF par_build_land_in is NOT NULL THEN
                BEGIN
                    /* Use long key */
                    CALL hpi.cpi_address_build_long_key(var_return_code,par_build_land_in, var_long_key);
                    SELECT
                        CONCAT(RTRIM(var_long_key), '%')
                        INTO var_long_key;
                    /*
                    ...
                    */
                    /* Use short key */
                    CALL hpi.cpi_address_build_short_key(var_return_code,par_build_land_in, var_short_key);
                    SELECT
                        CONCAT(LTRIM(RTRIM(var_short_key)), '%')
                        INTO var_short_key;
                    ...
                    */
                    /* Use soundex key */
                    CALL hpi.cpi_address_build_soundex_key(var_return_code,par_build_land_in, var_soundex_key);
            ...
            */
            IF NOT (par_street_in IS NULL) THEN
                BEGIN
                    /* Use long key */
                    CALL hpi.cpi_address_build_long_key(var_return_code,par_street_in, var_long_key);
                    /* print @long_key */
                    SELECT
                        CONCAT(var_long_key, '%')
                        INTO var_long_key;
                    ...
                    */
                    /* Use short key */
                    CALL hpi.cpi_address_build_short_key(var_return_code,par_street_in, var_short_key);
                    SELECT
                        CONCAT(LTRIM(RTRIM(var_short_key)), '%')
                        INTO var_short_key;
                    INSERT INTO t$temp_street
                    ...
                    */
                    /* Use soundex key */
                    CALL hpi.cpi_address_build_soundex_key(var_return_code,par_street_in, var_soundex_key);
                    INSERT INTO t$temp_street
                    SELECT DISTINCT
                        record_id, record_type, 1
                        FROM address_key_new
        ...
        */
        IF (par_location_all IS NULL) THEN
            BEGIN
                ...
                SELECT
                    ...
                    WHERE b.record_type IN ('V', 'I', 'A', 'G') AND a.short_key = CAST (b.record_id AS CHAR(7));
        ...
        ELSE
            BEGIN
                ...
                SELECT
                    ...
                    WHERE a.record_type = 'E' AND a.short_key = CAST (b.record_id AS CHAR(7)) AND a.record_id NOT IN (SELECT
                        record_id
                        FROM t$temp_final);
            END;
        END IF;
        ...
        SELECT
            ...
            END)) AS address_eng, CONCAT((CASE
            ...
            END), (CASE
                ...
                ELSE RTRIM(bldg_chi)
            END)) AS address_chi, aa.district_code, ad.district_name, (CASE
                WHEN aa.bldg_eng IS NULL THEN 'ZZZZZ'
                ELSE RTRIM(bldg_eng)
            END), (CASE
                WHEN aa.estate_eng IS NULL THEN 'ZZZZZ'
                ELSE RTRIM(estate_eng)
            END), aa.street_eng, aa.house_no, search_type
            FROM hpi.district AS ad, district_area AS ae, t$temp_final AS ab, address_detail AS aa
    ...
    OPEN p_refcur FOR
    SELECT
        record_id, address_eng, address_chi, district_code
        FROM t$temp_result
        ORDER BY search_type DESC NULLS FIRST, district_code ASC NULLS FIRST, estate_eng ASC NULLS FIRST, bldg_eng ASC NULLS FIRST, street_eng ASC NULLS FIRST, house_no ASC NULLS FIRST;
...
;ALTER FUNCTION "cpi_address_search" OWNER TO "HPI_SCHEMA_OWNER_ROLE";