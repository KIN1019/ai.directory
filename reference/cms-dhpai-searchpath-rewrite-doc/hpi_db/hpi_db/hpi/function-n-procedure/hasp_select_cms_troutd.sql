-- DROP FUNCTION hpi.hasp_select_cms_troutd(varchar, timestamp, timestamp, varchar, varchar, varchar);

CREATE OR REPLACE FUNCTION hpi.hasp_select_cms_troutd(par_hosp_code character varying, par_trans_sys_fdatetime timestamp without time zone, par_trans_sys_tdatetime timestamp without time zone, par_trans_ns_code character varying, par_from_time character varying, par_to_time character varying)
...
begin
    ...
    */
    ...
    SELECT
		...
		date_part('year', localtimestamp::TIMESTAMP) - date_part('year', p.dob::TIMESTAMP), date_part('month', localtimestamp::TIMESTAMP) - date_part('month', p.dob::TIMESTAMP), date_part('day', localtimestamp::TIMESTAMP) - date_part('day', p.dob::TIMESTAMP), localtimestamp,
		...
		Transaction_log.From_ward_code, Transaction_log.From_bed, Transaction_log.From_specialty_code, Transaction_log.From_class, Transaction_log.To_bed, Transaction_log.To_specialty_code, Transaction_log.To_class, Transaction_log.To_ward_code, 
		...
		DT_transaction_type.DT_code, par_TRANS_SYS_FDATETIME, par_TRANS_SYS_TDATETIME, par_FROM_TIME, par_TO_TIME, e.Doctor_code, e.Old_doctor_code, Transaction_log.Transaction_datetime, p.death_indicator, p.death_date
        FROM hpi.Transaction_log, hpi.Case_view AS c, hpi.cpi_patient AS p, hpi.Event_log AS e, hpi.DT_transaction_type
        WHERE 
        (Transaction_log.Hospital_code = par_hosp_code 
        	AND Transaction_log.Hospital_code = c.Hospital_code
        	AND Transaction_log.Case_no = c.Case_no
        	AND c.T_PRK = p.patient_key
        	...
        	AND Transaction_log.Transaction_type IN ('140', '220'))
...
;ALTER FUNCTION "hasp_select_cms_troutd" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
