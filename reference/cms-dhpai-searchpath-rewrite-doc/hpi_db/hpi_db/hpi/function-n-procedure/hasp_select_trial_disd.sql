-- DROP FUNCTION hpi.hasp_select_trial_disd(varchar, timestamp, timestamp, varchar, varchar, varchar);

CREATE OR REPLACE FUNCTION hpi.hasp_select_trial_disd(par_hosp_code character varying, par_trans_sys_fdatetime timestamp without time zone, par_trans_sys_tdatetime timestamp without time zone, par_trans_ns_code character varying, par_from_time character varying, par_to_time character varying)
    ...
    */
    ...
    SELECT
        Transaction_log.System_datetime, Transaction_log.Case_no, p.patient_name, p.sex, p.dob, c.discharge_code, c.destination_code, to_char(Transaction_log.System_datetime::TIMESTAMP WITHOUT TIME ZONE, 'HH24:MI:SS'), date_part('year', localtimestamp::TIMESTAMP) - date_part('year', p.dob::TIMESTAMP), date_part('month', localtimestamp::TIMESTAMP) - date_part('month', p.dob::TIMESTAMP), date_part('day', localtimestamp::TIMESTAMP) - date_part('day', p.dob::TIMESTAMP), localtimestamp, to_char(p.dob::TIMESTAMP WITHOUT TIME ZONE, 'DD-MM-YYYY'), par_TRANS_NS_CODE, to_char(par_TRANS_SYS_FDATETIME::TIMESTAMP WITHOUT TIME ZONE, 'DD/MM/YYYY'), Transaction_log.From_ward_code, Transaction_log.From_bed, Transaction_log.From_specialty_code, Transaction_log.From_class, Transaction_log.To_bed, Transaction_log.To_specialty_code, Transaction_log.To_class, CONCAT(to_char(Transaction_log.Transaction_datetime::TIMESTAMP WITHOUT TIME ZONE, 'DD/MM'), ' ', to_char(Transaction_log.Transaction_datetime::TIMESTAMP WITHOUT TIME ZONE, 'HH24:MI:SS')), Transaction_log.To_ward_code, (SELECT
            ...
            WHERE Transaction_log.Transaction_type = ADT_code), par_TRANS_SYS_FDATETIME, par_TRANS_SYS_TDATETIME, par_FROM_TIME, par_TO_TIME, Transaction_log.Transaction_datetime, p.death_indicator, p.death_date
        FROM Transaction_log, cpi_patient AS p, cpi_case AS c
        WHERE Transaction_log.Hospital_code = par_hosp_code AND Transaction_log.System_datetime >= par_TRANS_SYS_FDATETIME AND Transaction_log.System_datetime <= par_TRANS_SYS_TDATETIME AND Transaction_log.To_ward_code LIKE par_TRANS_NS_CODE AND Transaction_log.Transaction_type IN ('170', '240') AND c.hospital_code = Transaction_log.Hospital_code AND c.case_no = Transaction_log.Case_no AND p.patient_key = c.patient_key
...
;ALTER FUNCTION "hasp_select_trial_disd" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
