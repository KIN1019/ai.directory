-- DROP FUNCTION hpi."fn_tU_cpi_ae_case_detail"();

CREATE OR REPLACE FUNCTION hpi."fn_tU_cpi_ae_case_detail"()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
/* UPDATE trigger on cpi_ae_case_detail */
DECLARE
    var_numrows INTEGER;
    var_nullcnt INTEGER;
...
BEGIN
    ...
    BEGIN
        ...
        IF var_numrows = 0 THEN
            RETURN NULL;
        END IF;
        /* cpi_case Contains cpi_ae_case_detail ON CHILD UPDATE RESTRICT */
        IF update$case_no THEN
            BEGIN
                ...
                IF var_validcnt + var_nullcnt != var_numrows THEN
                    BEGIN
                        RAISE EXCEPTION '% % % ', 'UPDATE', 'cpi_ae_case_detail', 'cpi_case' USING ERRCODE := '200012';
                        EXIT error;
                    END;
                END IF;
            END;
...
;ALTER FUNCTION "fn_tU_cpi_ae_case_detail" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
