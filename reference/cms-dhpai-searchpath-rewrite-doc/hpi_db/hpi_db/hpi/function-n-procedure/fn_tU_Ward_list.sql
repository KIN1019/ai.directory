-- DROP FUNCTION hpi."fn_tU_Ward_list"();

CREATE OR REPLACE FUNCTION hpi."fn_tU_Ward_list"()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
/* UPDATE trigger on Ward_list */
DECLARE
    var_numrows INTEGER;
    var_nullcnt INTEGER;
...
BEGIN
    ...
    BEGIN
        ...
        IF var_numrows > 1 THEN
            BEGIN
                RAISE EXCEPTION '% % ', 'UPDATE', 'Ward_list' USING ERRCODE = '200013';
                EXIT error;
            END;
        END IF;
        ...
        IF NOT EXISTS (SELECT
            *
            ...
            BEGIN
                RAISE EXCEPTION '% % % ', 'UPDATE', 'Ward_list', 'cpi_movement' USING ERRCODE = '200018';
                EXIT error;
            END;
        END IF;
        RETURN NULL;
...
;ALTER FUNCTION "fn_tU_Ward_list" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
