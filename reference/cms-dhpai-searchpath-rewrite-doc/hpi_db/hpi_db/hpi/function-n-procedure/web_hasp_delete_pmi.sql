CREATE OR REPLACE PROCEDURE web_hasp_delete_pmi(INOUT pas_return_code integer, IN par_input_hkid VARCHAR, IN par_input_hosp_code VARCHAR, IN par_input_name VARCHAR, IN par_input_sex VARCHAR, IN par_input_dob TIMESTAMP WITHOUT TIME ZONE, IN par_input_ccc1 VARCHAR, IN par_input_ccc2 VARCHAR, IN par_input_ccc3 VARCHAR, IN par_input_ccc4 VARCHAR, IN par_input_ccc5 VARCHAR, IN par_input_ccc6 VARCHAR, IN par_input_exact_dob_flag VARCHAR, IN par_input_patient_key VARCHAR, IN par_system_datetime TIMESTAMP WITHOUT TIME ZONE, IN par_user_id VARCHAR, IN par_source_system VARCHAR, IN par_update_hosp VARCHAR)
 LANGUAGE plpgsql
AS 
$procedure$
DECLARE
        ...
        IF var_error != 0 THEN
            BEGIN
                SELECT
                    ...
                    INTO var_retcode;
                SELECT
                    'Unexpected error when selecting Hospital_control'
                    INTO var_error_msg;
                EXIT end_error;
            END;
        END IF;
        ...
        IF var_rowcount <> 1 THEN
            BEGIN
                SELECT
                    ...
                    INTO var_retcode;
                SELECT
                    'Incorrect stored proc for CPI server'
                    INTO var_error_msg;
                /* --raiserror @retcode  "Incorrect stored proc for CPI server" */
                EXIT end_error;
            END;
        END IF;
        ...
        IF var_retcode != 0 THEN
            BEGIN
                SELECT
                    ...
                    INTO var_retcode;
                SELECT
                    'Fail to insert Event_log, PMI delete rejected'
                    INTO var_error_msg;
                /* --raiserror 29999, "Fail to insert Event_log, PMI delete rejected" */
                EXIT end_error;
            END;
        END IF;
    END;
...
;ALTER PROCEDURE "web_hasp_delete_pmi" OWNER TO "HPI_SCHEMA_OWNER_ROLE";
