---
title: CMS DHPAI Sonarqube Bug Resolving Instructions
description: |
  Project path: "<input your sonarqube project key eg cms-pccms-svc>"
  SonarQube server: sonarqube-mcp
  Git operation: github mcp (CMS organization)
  Target branch for PR: "<input the branch you want to merge into, e.g., main or master>"
tags: [dhpai, sonarqube, bugfix, ci]
---

## Project Request

**Task**: Complete SonarQube bug fixing to resolve all bug issues
...
**Workflow Required**: Please run the following workflow based on SonarQube report. Complete ALL bug issues with ALL severities in the order specified:

## INITIAL SETUP

**Git Branch Creation (Do this FIRST before any fixes)**:
- Create new branch: `sonarqube-fix/fix-bug_YYYYMMDDHHMM` (use current datetime)
- Switch to this branch for all subsequent work

## PHASE 2: Bug Issues (Fix ALL Severities)
**IMPORTANT: Do NOT push yet - this is the final phase before creating single PR**

Please follow the steps below to fix Bug issues type:
1. **Direct Issue Processing**
   - **Step 1: Fetch BLOCKER Bug Issues** - Use SonarQube MCP to get BLOCKER Bug issues: `mcp_sonarqube_issues` with parameters:
     * `types`: ["BUG"]
   ...
   - **Step 3: Fetch CRITICAL Bug Issues Page-by-Page** - Use SonarQube MCP:
   ...
   - **Step 4: Fetch MAJOR Bug Issues Page-by-Page** - Use SonarQube MCP:
     * **Page 1**: `mcp_sonarqube_issues` with same parameters but `severities`: ["MAJOR"]
     * **Fix Page 1 Issues**: Process all 50 issues from page 1 response immediately
     * **Page 2**: Fetch page 2 and fix those issues
     * **Continue**: Repeat until all MAJOR pages are processed
     * **MAJOR**: Fix issues directly from each API response

   - **Step 5: Fetch MINOR Bug Issues Page-by-Page** - Use SonarQube MCP:
     * **Page 1**: `mcp_sonarqube_issues` with same parameters but `severities`: ["MINOR"]
     * **Fix Page 1 Issues**: Process all 50 issues from page 1 response immediately
     * **Page 2**: Fetch page 2 and fix those issues
     * **Continue**: Repeat until all MINOR pages are processed
     * **MINOR**: Fix issues directly from each API response

2. **Analysis & Issue Identification**
   - Process issues directly from SonarQube API responses
   - **IMPORTANT**: Fix ALL severity bug issues (BLOCKER, CRITICAL, MAJOR, MINOR)
   - **FIX ALL**: Process all bug severities systematically

3. **Bug Fixing Priority (ALL Severities) - Direct Processing**
   - **Optimized Workflow**:
     * **Phase 3a: Process BLOCKER Bug Issues (if any)**
       - Fix each blocker issue immediately from API response
     * **Phase 3b: Process CRITICAL Bug Issues Page-by-Page**
     ...
     * **Phase 3c: Process MAJOR Bug Issues Page-by-Page**
       - Fetch page 1, fix all issues in that page, do not fix next page until you have finish all issue in that page
       - Fetch page 2, fix all issues in that page, do not fix next page until you have finish all issue in that page
       - Continue until all MAJOR pages are processed
     * **Phase 3d: Process MINOR Bug Issues Page-by-Page**
       - Fetch page 1, fix all issues in that page, do not fix next page until you have finish all issue in that page
       - Fetch page 2, fix all issues in that page, do not fix next page until you have finish all issue in that page
       - Continue until all MINOR pages are processed
   ...
   - **FIX ALL SEVERITIES**: Process ALL bug severity levels (BLOCKER, CRITICAL, MAJOR, MINOR, INFO)
   - **Benefit**: No file I/O issues, direct processing, clear progress tracking, comprehensive bug resolution
...
5. **Phase 2 Commit (Individual commit for Bug fixes)**
   - Stage all Phase 2 changes: `git add -A`
   - Commit with descriptive message: "fix: resolve all SonarQube bug issues (all severities)"
