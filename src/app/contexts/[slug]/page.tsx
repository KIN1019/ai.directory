	...
	const fetchLlmsContent = async (
		...
		try {
			...
			if (llmsResponse.ok) {
				...
				setLlmsContent(
					`Failed to load content: ${llmsData.error || "Unknown error"}`,
				);
			}
		} catch (err) {
	...
	if (error || !library) {
		return (
			<div className="flex flex-col justify-center items-center h-[90vh] gap-4">
				...
				<Button onClick={() => router.push("/contexts")}>
					Back to Libraries
				</Button>
			</div>
		);
			...
			<div className="mb-8">
				...
				<div className="border rounded-lg p-6 space-y-6">
					...
					<p className="text-xs text-muted-foreground">
						💡 Adjust topic and token size, then click <strong>Search</strong>{" "}
						to fetch filtered documentation from the DHP AI Code Context API.
					</p>
				</div>
			</div>
