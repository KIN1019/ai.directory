...
export default function SkillsPage() {
	...
	>((acc, skill) => {
		if (!acc[skill.source]) acc[skill.source] = { skills: [], buckets: new Set() };
	...
	return (
		<div className="p-8 pb-32">
			...
			{Object.entries(grouped).map(([source, { skills: sourceSkills, buckets }]) => (
					...
					<div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
						...
						<div className="flex gap-2 flex-wrap">
							{Array.from(buckets).map((bucketPath) => (
								...
								>
									<a
										href={`/api/skills/download?path=${encodeURIComponent(bucketPath)}`}
										download
									>
										<Download className="w-3.5 h-3.5" />
										Download All ({sourceSkills.filter((s) => s.skillsDirPath === bucketPath).length} skills)
									</a>
								</Button>
							))}
						</div>
